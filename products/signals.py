...
def subscription_created(sender, **kwargs):
    ...
    year = last_purchase.license_end.year - int(last_purchase
                                                .product.license_type[0])
    ...
    if ipn_obj.payment_status == "Completed":
        ...
        if ipn_obj.mc_gross != last_purchase.product.price \
                or ipn_obj.mc_currency != 'GBP':
            return

        # Check if the purchase was already saved in the database
        # (duplicate signal), and if not, save it
        if int(last_purchase.user_id) == int(user_id) \
                and int(last_purchase.product_id) == int(product_id):
            if last_purchase_date > arrow.utcnow()\
                    .replace(seconds=-60).datetime:
                return

        license_duration = int(models.Product.objects.get(pk=product_id)
                               .license_type[0])
        models.Purchase.objects.create(product_id=product_id, user_id=user_id,
                                       license_end=arrow.now().replace(
                                           years=+license_duration,
                                           tzinfo='Europe/London').datetime)
